  ...
  module.exports = function(watchGroup) {
    if (watchGroup == null) watchGroup = this;
    return watchGroup.on('new-stat', function() {
      ...
      if (m > 0) watchGroup.kill(m);
      if (n > 0) watchGroup.spawn(n);
      if (watchGroup.total_mem > watchGroup.maxMemory) {
        watchGroup.emit('mem-exceed');
      }
      if (watchGroup.total_icpu > watchGroup.maxCpu) watchGroup.emit('cpu-exceed');
